...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 26 16:54:49 2017
...
path = '/home/liushuanglong/MyFiles/Data/JYDB2/LC_IncomeStatementNew/LC_IncomeStatementNew_mat.mat'
dataRaw = sio.loadmat(path)  
dataColTol = [dataRaw['col'][0][i][0] for i in range(dataRaw['col'].shape[1])]
dataColUseLis = [u'InfoPublDate', u'CompanyCode', u'EndDate', u'Mark', u'NetProfit']
dataColUseInd = [dataColTol.index(i) for i in dataColUseLis]
dataIncomeTolArr = dataRaw['data'][:, dataColUseInd]


# dataframe, select by company code and mark
dataIncomeTolDF = pd.DataFrame(dataIncomeTolArr, columns=dataColUseLis)    # 923302
dataIncomeUseDF = dataIncomeTolDF[dataIncomeTolDF[u'CompanyCode'].isin(dataComCode)]  # 667895
dataIncomeUseDF = dataIncomeUseDF[dataIncomeUseDF[u'Mark'].isin([1, 2])]      # 293845
dataIncomeUseDF = dataIncomeUseDF[dataIncomeUseDF[u'NetProfit'].notnull()]    #293805 


# sort by company code , published date, end date, mark
dataIncomeUseDFSor = dataIncomeUseDF.sort_values(by=[u'CompanyCode', u'InfoPublDate', u'EndDate', u'Mark'], \
                                                 ascending=[True, True, True, False])
#dataComCodeUse = dataIncomeUseDF[u'CompanyCode'].drop_duplicates().values     # used company code, 3364 counts
dataComCodeUse = np.unique(dataIncomeUseDF[u'CompanyCode'])             # used company code, 3364 counts

# create  df dict
dataIncomeUseDFInd = dataIncomeUseDFSor.set_index([u'CompanyCode', u'InfoPublDate'])
dataIncomeDic = {}

for code in dataComCodeUse:
    dataIncomeDic[code] = dataIncomeUseDFInd.loc[code]
...
for code in dataComCodeUse:    
    dfOneCode = dataIncomeDic[code]                              # df one code
    ...
    for day in pubDateOneCodeArr:
        ...
        if 1231 not in EndDateDayLis:
            ...
            yearEndDayInd = np.array([yearEndDayInd]).reshape(np.array([yearEndDayInd]).size).tolist()
...
#                    dayUse = dfOneCodeUseTemp.index
        ...
        #                    dayUse = dfOneCodeUseTemp.index
                    ...
                    if count == len(newestYearDay):
                        ...
                        dfOneCodeUse.loc[day] = dfOneCodeUseTemp.values.reshape(3,)
    print code                    
    dataIncomeDicUse[code] = dfOneCodeUse                       
...
#
#cp.dump(dataIncomeDicUse, open("/home/liushuanglong/MyFiles/Data/Factors/HLZ/CompanyFundamentalData/LC_IncomeStatementNew_NetProfit_YearDuration_DicUse.pkl", "w")) 
...
##
##data111 = cp.load(open("/home/liushuanglong/MyFiles/Data/Factors/HLZ/CompanyFundamentalData/LC_IncomeStatementNew_YearDuration_DicUse.pkl", 'r'))
##
#
## load the time series and fillna
#    
#pathTS =  '/home/liushuanglong/MyFiles/Data/Factors/HLZ/Time&Code/alltdays_mat.mat'     
#dataTSRaw = sio.loadmat(pathTS)      
#dataTSArr = dataTSRaw['data'][:, 0]
#    
...
#dataComCodeSer = pd.Series(dataComCodeArr)
#dataComCodeSerSet = dataComCodeSer.drop_duplicates()
